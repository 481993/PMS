...
if (!function_exists('show_column_value')) {
    ...
    //    print_r($column);
    //    die; 
        if($column->Field == 'departments_name')
        {
            $column->Field = 'departments_id';
        }
    
                    if($column->Field == 'Project name')
                    {
                        $column->Field = 'project_id';
                    }
                    if($column->Field == 'Milestone name')
                    {
                        $column->Field = 'milestone_id';
                    }
                    if($column->Field == 'Employee name')
                    {
        ...
        // $a=$valueObject->id;
        // echo '<pre>';
         //print_r(get_class($valueObject));
         $module_name=get_class($valueObject);
        ...
        if (($column_type == 'date') && $value != '') {
        ...
        } elseif ( $column_name == 'created_by' || $column_name == 'updated_by' ) {
            $qrycreateby = 'SELECT `name` FROM `users` WHERE id = '.$value;
            $res = DB::select($qrycreateby);
            //print_r($dept);
            if($res){
                $return_text = $res[0]->name;
            }
        } elseif ( $column_name == 'project_type' ) {
            $qrycreateby = 'SELECT `name` FROM `projecttypes` WHERE id = '.$value;
            $res = DB::select($qrycreateby);
            //print_r($dept);
            if($res){
                $return_text = $res[0]->name;
            }
        } elseif ( $column_name == 'departments_id' ) {
            $qrycreateby = 'SELECT `name` FROM `departments` WHERE id = '.$value;
            $res = DB::select($qrycreateby);
            // print_r($res);
            if($res){
                $return_text = $res[0]->name;
            }
        } elseif ( $column_name == 'project_manager' ) {
            $qrycreateby = 'SELECT `name` FROM `employees` WHERE id = '.$value;
            $res = DB::select($qrycreateby);
            //print_r($dept);
            if($res){
                $return_text = $res[0]->name;
            ...
            if($value == '1'){
                $return_text = 'Published';
            }
            elseif ($value == '2') {
                $return_text = 'Draft';
            }
            elseif ($value == '0') {
                $return_text = 'Unpublished';
            }
        } elseif ( $column_name == 'priority' ) {
            if($value == '1'){
                $return_text = 'High';
            }
            elseif ($value == '2') {
        ...
        elseif($column_name == 'project_id')
        {
            $qrycreateby = 'SELECT `name` FROM `projects` WHERE id = '.$value;   
            $res = DB::select($qrycreateby);
        ...
        elseif($column_name == 'milestone_id')
        {
            $qrycreateby = 'SELECT `name` FROM `milestones` WHERE id = '.$value;
            $res = DB::select($qrycreateby);
            // print_r($res);
            if($res){
                $return_text = $res[0]->name;
        ...
        elseif($column_name == 'employee_id')
        {
            $qrycreateby = 'SELECT `name` FROM `employees` WHERE id = '.$value;
            $res = DB::select($qrycreateby);
            // print_r($res);
        ...
        elseif($column_name == 'task_type')
        {
            $qrycreateby = 'SELECT `name` FROM `tasktypes` WHERE id = '.$value;
            $res = DB::select($qrycreateby);
            // print_r($res);
            if($res){
                $return_text = $res[0]->name;
        ...
        elseif ( $column_name == 'task_priority' ) {
            if($value == '1'){
                $return_text = 'High';
            }
            elseif ($value == '2') {
        ...
        elseif ( $column_name == 'task_status' ) {
            if($value == '0'){
                $return_text = 'New';
            }
            elseif ($value == '1') {
                $return_text = 'Inprogress';
            }
            elseif ($value == '2') {
                $return_text = 'Closed';
            }
        ...
        elseif($column_name == 'Total task' && $module_name == 'Modules\Project\Entities\Project')
        {
            $projectid=$valueObject->id;
            $value = $valueObject->$column_name;
        $totaltask ='SELECT count(`id`) AS total_task FROM `tasks` WHERE project_id='.$projectid; 
        $task= DB::select($totaltask);
        $return_text= $task[0]->total_task;

        }
        elseif($column_name == 'Total employee' && $module_name == 'Modules\Project\Entities\Project')
        {
            $projectid=$valueObject->id;
            $value = $valueObject->$column_name;
           $totalemployee= 'SELECT count(employee_id) AS total_employee FROM `projectassigns` WHERE project_id = '.$projectid;
           $total_emp= DB::select($totalemployee);
          // print_r($t);

        //    foreach ($t as $value) {
        //     var_dump($value);
        //     }
            $return_text= $total_emp[0]->total_employee;
        //    print_r($t); 
        //    if($t){
        //     $return_text = $t;
        //      }
        ...
        elseif($column_name == 'Total milestone' && $module_name == 'Modules\Project\Entities\Project')
        {

            $projectid=$valueObject->id;
            $value = $valueObject->$column_name;
        $totalmilestone ='SELECT count(project_id) AS total_milestone FROM `milestones` WHERE project_id = '.$projectid; 
        $total_milestone= DB::select($totalmilestone);
        $return_text= $total_milestone[0]->total_milestone;

        }
        elseif($column_name == 'Log Name'  && $module_name == 'Modules\Timelog\Entities\Timelog')
        {
           
            $value = $valueObject->$column_name;
            $projectid=$valueObject->id;
           $logname ='SELECT `name` FROM `timelogs` WHERE id = '.$projectid;
            //die();
            $log_name= DB::select($logname);
            $return_text= $log_name[0]->name;
            // var_dump($return_text);
            // die();
            //var_dump($time);
           // die();
            //$return_text= $time[0]->name;
        }
        elseif($column_name == 'Task Name' && $module_name == 'Modules\Timelog\Entities\Timelog')
        {
        $value = $valueObject->$column_name;
        $projectid=$valueObject->id;
        $taskname ='SELECT t.`name` FROM `tasks`  AS t left join `timelogs` AS tl ON tl.task_id=t.id WHERE  tl.id = '.$projectid; 
        // var_dump($taskname);
        $task_name= DB::select($taskname);
        $return_text= $task_name[0]->name;
        }
