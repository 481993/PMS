...
class ResourcesController extends BackendBaseController
{
    ...
    public function get_task_detail()
    {
        ...
        if(isset($$module_name) && !empty($$module_name)) {
            foreach($$module_name as $key => $value){
                ...
                if(is_float($per_day_hour)){
                    ...
                    if($per_day_hour < 8 && $per_day_hour!=8){
                        if(isset($whole) && !empty($whole)){
                            ...
                            $new_end_dt = $this->sumDays($whole-1, 'Y-m-d', $value->startDate);
                        }
                        // elseif ($whole == 0) {
                        ...
                        // }
                            ...
                            $new_endDate = $this->sumDays($per_day_hour-1, 'Y-m-d', $value->startDate);
                        }
                        // var_dump($inc_day);
                        // var_dump($value->startDate);
                        // $new_end_dt = strtotime($inc_day, $value->startDate);
                        // var_dump($new_end_dt);
                        // echo $effectiveDate = date('Y-m-d', strtotime($inc_day, strtotime($value->startDate)));
                ...
                // var_dump(isset($new_end_dt) && !empty($new_end_dt));
                if(isset($new_end_dt) && !empty($new_end_dt) && $new_end_dt != $value->startDate){
                    $v1 = (array) $value;
                    $v1['name'] = strtolower(str_replace(' ', '_', $value->name));
                    $v1['customClass'] = 'redHClass greenHClass '.$partial_class;
                    $v1['title'] = 'booked';
                    $v1['startDate'] = $this->sumDays(1, 'Y-m-d', $new_end_dt); //date('Y-m-d', strtotime('+1 day', strtotime($new_end_dt)));
                    $v1['endDate'] = $this->sumDays(1, 'Y-m-d', $new_end_dt); //date('Y-m-d', strtotime('+1 day', strtotime($new_end_dt)));
                    // var_dump($this->sumDays(4, 'Y-m-d', $new_end_dt));
                    $cnt++;
                ...
                //$weekends_arr[] = array('id' => '101'.$cnt, 'name' => $v1['name'], 'startDate' => date('Y-m-d', $date), 'endDate' => date('Y-m-d', $edate), 'customClass' => 'grayClass', 'title'=>'Vacation');
                // while (date("Y-m-d", $date) != date("Y-m-d", $edate)) {
                //     $day_index = date("w", $date);
                //     if ($day_index == 0 || $day_index == 6) {
                //         // Print or store the weekends here
                //         $cnt++;
                //         $weekends[] = array('id' => $cnt, 'name' => $v1['name'], 'startDate' => date('Y-m-d', $date), 'endDate' => date('Y-m-d', $date), 'customClass' => 'grayClass', 'title'=>'Vacation');
                //     }
                //     $date = strtotime(date("Y-m-d", $date) . "+1 day");
                // }
                $cnt++;
                $carr[] = array('id' => $cnt, 'name' => $v1['name'], 'startDate' => date('Y-m-d', $date), 'endDate' => date('Y-m-d', $edate), 'customClass' => 'greenClass', 'title'=>'Available' );
                $cnt++;
            }
        }
        // echo '<pre>';
        ...
        if(isset($obj_data1) && !empty($obj_data1)){
            ...
            foreach($data1 as $value){
                ...
                $available_emp[] = array('id' => $cnt1, 'name' => $str_emp_name, 'startDate' => date('Y-m-d', $date), 'endDate' => date('Y-m-d', $edate), 'customClass' => 'greenClass', 'title'=>'Available' );
                $cnt1++;
            }
    ...
    public function addDays($days,$format="Y-m-d"){
